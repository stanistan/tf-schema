...
func (p *{{.NS}}Provider) AsProvider() *tfschema.Provider {
	var configure tfschema.ConfigureContextFunc
	if p.ConfigureFunc != nil {
		configure = func(ctx context.Context, d *tfschema.ResourceData) (interface{}, diag.Diagnostics) {
			return p.ConfigureFunc(ctx, d)
		}
	}
...
func {{.Signatures.singleReturn.WrapFn .NS}} (
	name string,
	exec func(context.Context, *tfschema.ResourceData, *{{.Client}}) diag.Diagnostics,
) func(context.Context, *tfschema.ResourceData, interface{}) diag.Diagnostics {
	if exec == nil {
		return nil
	}
	...
	return func(ctx context.Context, d *tfschema.ResourceData, meta interface{}) diag.Diagnostics {
		c, ok := meta.(*{{.Client}})
		if !ok {
		  return diag.FromErr(fmt.Errorf("meta misconfigured -- expected {{.Client}}, cannot execute %s", name))
		}
